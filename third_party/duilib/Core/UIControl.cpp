...
CDuiString CControlUI::LoadString(LPCTSTR pstrID)
{
	CMarkup xmlStrTable;
	CDuiString strXml = GetManager()->GetStringTableXml();

	if(!strXml.IsEmpty() && !xmlStrTable.LoadStringTableFile(strXml))
		return pstrID;

	CMarkupNode root = xmlStrTable.GetRoot();
    LPCTSTR strRootName = root.GetName();
	if(strRootName && _tcscmp(strRootName, _T("StringTable")))
		return pstrID;
	...
	{
		pstrName = node.GetName();
		if(!_tcscmp(pstrName, _T("String")))
		{
			...
			while (i < nAttributes)
			{
				...
				if (!_tcscmp(pstrName, _T("id")) && !_tcscmp(pstrValue, pstrID))
				{
					while (i < nAttributes)
					{
						++i;
...
void CControlUI::SetText(LPCTSTR pstrText)
{
	...
	// Add By Clouderman. 2013-1-8 16:16:08
	if (_tcslen(pstrText) >= 2 && pstrText[0] == _T('#') && pstrText[1] != _T('#'))
    {
        CDuiString sText = LoadString(pstrText + 1);
		if (sText == pstrText)
            return;
        else
            m_sText = sText;
...
void CControlUI::SetToolTip(LPCTSTR pstrText)
{
	// Add By Clouderman. 2013-1-8 16:16:08
	if ( _tcslen(pstrText) >= 2 && pstrText[0] == _T('#') && pstrText[1] != _T('#') )
		m_sToolTip = LoadString(pstrText + 1);
	else
		m_sToolTip = pstrText;
}
...
CControlUI* CControlUI::ApplyAttributeList(LPCTSTR pstrList)
{
    ...
    while( *pstrList != _T('\0') ) {
        ...
        while( *pstrList != _T('\0') && *pstrList != _T('=') ) {
            ...
            if( pstrList < pstrTemp ) {
                if( !_istspace(*pstrList) )
                    sItem += *pstrList;
                pstrList++;
            }
        }
        if ( *pstrList == _T('\0')) return this;
        ASSERT( *pstrList == _T('=') );
