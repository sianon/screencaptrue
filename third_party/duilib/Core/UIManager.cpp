...
CDuiString CPaintManagerUI::m_strStringTableZip;
HANDLE CPaintManagerUI::m_hStringTableZip = NULL;

bool CPaintManagerUI::m_bForceUseLocalResourceXml = false;
bool CPaintManagerUI::m_bForceUseLocalResourceImage = false;
bool CPaintManagerUI::m_bForceUseLocalResourceStringTable = false;
...
const CDuiString& CPaintManagerUI::GetStringTableZip()
{
	return m_strStringTableZip;
}

HANDLE CPaintManagerUI::GetStringTableZipHandle()
{
	return m_hStringTableZip;
}

bool CPaintManagerUI::IsForceUseLocalResourceXml()
{
  return m_bForceUseLocalResourceXml;
}

bool CPaintManagerUI::IsForceUseLocalResourceImage()
{
  return m_bForceUseLocalResourceImage;
}

bool CPaintManagerUI::IsForceUseLocalResourceStringTable()
{
  return m_bForceUseLocalResourceStringTable;
}
...
void CPaintManagerUI::SetStringTableZip(LPVOID pVoid, unsigned int len)
{
	if( m_strStringTableZip == _T("membuffer") )
		return;
	if( m_hStringTableZip != NULL )
	{
		CloseZip((HZIP)m_hStringTableZip);
		m_hStringTableZip = NULL;
	}
	m_strStringTableZip = _T("membuffer");
	m_hResourceZip = (HANDLE)OpenZip(pVoid, len, 3);
}

void CPaintManagerUI::SetStringTableZip(LPCTSTR pstrZip)
{
	if( m_strStringTableZip == pstrZip )
		return;
	if( m_hStringTableZip != NULL )
	{
		CloseZip((HZIP)m_hStringTableZip);
	...
	m_strStringTableZip = pstrZip;
	CDuiString sFile = CPaintManagerUI::GetResourcePath();
	sFile += CPaintManagerUI::GetStringTableZip();
	m_hStringTableZip = (HANDLE)OpenZip((void*)sFile.GetData(), 0, 2);
}

void CPaintManagerUI::SetForceUseLocalResourceFile(bool image, bool xml, bool string_table)
{
  m_bForceUseLocalResourceImage = image;
  m_bForceUseLocalResourceXml = xml;
  m_bForceUseLocalResourceStringTable = string_table;
}
    ...
    case WM_PAINT:
        {
            ...
            if( m_bOffscreenPaint )
            {
                ...
                SendNotify(m_pRoot, _T("onpaint"), (WPARAM)m_hDcOffscreen, (LPARAM)&ps.rcPaint, false);
...
CDuiString CPaintManagerUI::GetStringTableXml()
{
	return m_strStringTableXml;
}

void CPaintManagerUI::SetStringTableXml(LPCTSTR pstrLanguageXml)
{
	m_strStringTableXml = pstrLanguageXml;
}
...
bool CPaintManagerUI::TranslateMessage(const LPMSG pMsg)
{
	...
	if (uChildRes != 0)
	{
		HWND hWndParent = ::GetAncestor(pMsg->hwnd, GA_PARENT);
		...
		for( int i = 0; i < m_aPreMessages.GetSize(); i++ ) 
		{
			...
			while(hTempParent)
			{
				...
				hTempParent = ::GetAncestor(hTempParent, GA_PARENT);
			}
		}
	}
	else
