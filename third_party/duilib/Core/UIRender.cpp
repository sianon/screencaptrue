...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
	...
	do 
	{
      ...
      if (CPaintManagerUI::GetResourceZip().IsEmpty() || CPaintManagerUI::IsForceUseLocalResourceImage()) {
                                          ...
                                          LPCTSTR pStrImage, LPCTSTR pStrModify)
{
    ...
    for( int i = 0; i < 2; ++i,image_count = 0 ) {
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
                ...
                else if( sItem == _T("center") ) {
                    bCenter = (_tcscmp(sValue.GetData(), _T("true")) == 0);
                }
            }
            if( *pStrImage++ != _T(' ') ) break;
        }
    ...
    if (bCenter)
    {
        bTiledX = bTiledY = false;

        LONG nWidth = rcBmpPart.right - rcBmpPart.left;
        LONG nHeight= rcBmpPart.bottom - rcBmpPart.top;

        rcItem.left = rc.left + (rc.right - rc.left - nWidth) / 2;
        rcItem.right = rcItem.left + nWidth;
        rcItem.top = rc.top + (rc.bottom - rc.top - nHeight) / 2;
            ...
            case _T('a'):  // Link
                {
                    ...
                    if( pFontInfo->bUnderline == false ) {
                        ...
                        if( hFont == NULL ) hFont = pManager->AddFont(pFontInfo->sFontName, pFontInfo->iSize, pFontInfo->bBold, false, pFontInfo->bItalic);
