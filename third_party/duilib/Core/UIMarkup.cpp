...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
    ...
    if (CPaintManagerUI::GetResourceZip().IsEmpty() || CPaintManagerUI::IsForceUseLocalResourceXml()) {
        sFile += pstrFilename;
...
bool CMarkup::LoadStringTableFile( LPCTSTR pstrFilename, int encoding /*= XMLFILE_ENCODING_UTF8*/ )
{
	Release();
	CDuiString sFile = CPaintManagerUI::GetResourcePath();

  if (CPaintManagerUI::GetStringTableZip().IsEmpty() || CPaintManagerUI::IsForceUseLocalResourceStringTable())
	{
		sFile += pstrFilename;
		HANDLE hFile = ::CreateFile(sFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	else
	{
		...
		HZIP hz = (HZIP)CPaintManagerUI::GetStringTableZipHandle();
		if( hz == NULL )
