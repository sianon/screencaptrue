...
void CWindowWnd::Close(UINT nRet/* = IDOK*/)
{
    ASSERT(::IsWindow(m_hWnd));
    if( !::IsWindow(m_hWnd) )
		return;
    PostMessage(WM_CLOSE, nRet, 0L);
}

bool CWindowWnd::CenterWindow( HWND hWndCenter )
{
	ASSERT(::IsWindow(m_hWnd));
...
BOOL CWindowWnd::ModifyStyle(DWORD dwRemove, DWORD dwAdd, UINT nFlags/* = 0*/)
{
    ASSERT(::IsWindow(m_hWnd));
...
BOOL CWindowWnd::ModifyStyleEx(DWORD dwRemove, DWORD dwAdd, UINT nFlags/* = 0*/)
{
    ASSERT(::IsWindow(m_hWnd));
