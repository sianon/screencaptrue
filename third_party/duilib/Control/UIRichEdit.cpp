...
public:
    ...
    BOOL SetOleCallback(IRichEditOleCallback* pCallback)
    {
        if (NULL == pserv)
            return FALSE;
        HRESULT lRes = 0;
        pserv->TxSendMessage(EM_SETOLECALLBACK, 0, (LPARAM)pCallback, (LRESULT*)&lRes);
        return (BOOL)lRes;
    }
    ...
    BOOL CanPaste(UINT nFormat = 0)
    {
        if (NULL == pserv)
            return FALSE;
        HRESULT lRes = 0;
        pserv->TxSendMessage(EM_CANPASTE, nFormat, 0L, (LRESULT*)&lRes);
        return (BOOL)lRes;
    }
...
{
	// Modify By Clouderman. 2013-1-8 16:16:08
